...
class User extends Authenticatable
{
     ...
     */
    protected $fillable = [
        'stripe_cus_id', 'user_group_id', 'name', 'username', 'slug', 'email', 'mobile_no', 'password', 'image_url', 'image_url_blur', 'cover_image_url', 'business_category', 'platform_id', 'platform_type', 'business_name', 'business_timings', 'about_business', 'status', 'is_email_verify', 'email_verify_at', 'is_mobile_verify', 'mobile_verify_at','is_approve','approve_at', 'country', 'city', 'state', 'zipcode', 'address', 'latitude', 'longitude', 'online_status', 'rating', 'total_ratings', 'total_reviews', 'mobile_otp', 'email_otp','total_earnings','total_remaining','remember_token', 'created_at', 'updated_at', 'deleted_at'
    ];
    ...
    {
        return $this->belongsTo(ProviderTransaction::class, 'provider_id', 'id');
    }
    ...
    {
        return $this->hasMany(Review::class, 'provider_id','id')->orderBy('created_at', 'desc')->take(3);
    }

    public function userKeyWords(): \Illuminate\Database\Eloquent\Relations\HasMany
    {
    ...
    public function businessCategory(): \Illuminate\Database\Eloquent\Relations\HasOne
    {
        return $this->hasOne(Category::class, 'id','business_category');
    }

    public function userApiToken(): \Illuminate\Database\Eloquent\Relations\HasMany
    {
    ...
    {
        return $this->hasMany(Order::class,'user_id','id')->where('user_group_id', 1);
    }

    public function providerOrders(): \Illuminate\Database\Eloquent\Relations\HasMany
    {
        return $this->hasMany(OrderItem::class,'provider_id','id')->where('user_group_id', 2);
    }

    public function cashOutRequest(): \Illuminate\Database\Eloquent\Relations\HasMany
    {
    ...
    public function userCards(): \Illuminate\Database\Eloquent\Relations\HasMany
    {
     ...
     */
    public static function generateUniqueUserName($username): string
    {
    ...
    public static function getUserByApiToken($api_token)
    {
        ...
        $user = self::select('users.*', 'cat.title as business_category')
                    ->selectRaw('api_token,device_type,device_token,uat.platform_type,uat.platform_id')
...
//                    ->selectRaw("CONCAT('$base_url', users.cover_image_url) as cover_image_url")
    ...
    public static function emailExists($param){
    ...
    public static function phoneExists($param){
        $user = self::where('mobile_no',$param['mobile_no'])->first();
        if($user){
            return ['isPhoneNoExist' => true];
        }else{
            return ['isPhoneNoExist' => false];
        }
    }

    public static function socialUser($params)
    {
//        dd($params['platform_id']);
        $data = new \stdClass();
        ...
        if( !empty($params['image_url']) ){
            $name = substr($params['image_url'], strrpos($params['image_url'], '/') + 1);
            $image_url = file_get_contents($params['image_url']);
            $filename = file_put_contents(public_path('storage/users/' . $name),$image_url);
            $image_url = 'users/'.$name;
        }else{
            $image_url = null;
        }
        //create new user
    ...
    public function drivingLicense(){
        ...
        return $this->hasMany(Media::class, 'module_id','id')
            ->select('media.*')
            ->selectRaw("CONCAT('$base_url', file_url) as file_url")
            ->where('module','User License');
    }
}

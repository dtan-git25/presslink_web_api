...
class UserHook
{
   ...
   */
    public function hook_query_index(&$query,$request, $slug=NULL) {
        ...
        if($routeName == "provider.ajax-listing" || $routeName == "provider-earning.ajax-listing"){
            $query->where('user_group_id',2);
        }else{
            $query->where('user_group_id', '!=', 2);
        }
    ...
    */
    public function hook_before_edit($request, $slug, &$postData)
    {
        ...
        if(isset($postData['is_approve']) && $postData['is_approve'] == 1){
            $user->update(['is_approve'=> $request->is_approve, 'approve_at' => Carbon::now()]);
        }
    }

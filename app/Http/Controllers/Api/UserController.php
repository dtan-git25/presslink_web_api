...
class UserController extends RestController
{
    ...
    public function validation($action,$slug=NULL)
    {
        ...
        $custom_messages = [
            ...
            'keywords.between' => __('app.keyword_field_required_max'),
        ...
        switch ($action){
            case 'POST':
                $validator = Validator::make($this->__request->all(), [
                    ...
                    'cover_image_url' => 'sometimes|required',
                    'business_category' => 'sometimes|required',
                    'business_name' => 'sometimes|required',
                    'business_timings' => 'sometimes|required',
                    'about_business' => 'sometimes|required',
                    'address' => 'sometimes|required',
                    'country' => 'sometimes|required',
                    'city' => 'sometimes|required',
                    'state' => 'sometimes|required',
                    'zipcode' => 'sometimes|required',
                    'keywords' => 'sometimes|required|between:1,5',
                ],$custom_messages);
                break;
            case 'PUT':
                $custom_messages = [
                    'slug.exists' => __('app.invalid_user'),
                    'keywords.between' => __('app.keyword_field_required'),
                    'image_url.max' => __('app.image_upload_max_limit')
                ];
                $this->__request->merge(['slug' => $slug]);
                $validator = Validator::make($this->__request->all(), [
                    'slug'          => 'exists:users,slug',
                    'name'          => ['sometimes','required','min:3','max:50','regex:/^([A-Za-z0-9\s])+$/'],
                    'email'         => 'sometimes|required|email|unique:users,email,'.$this->__request['user']->id,
//                        Rule::unique('users')->where('id', $this->__request['user']->id)->whereNull('deleted_at')
                    'image_url' => 'sometimes|required|mimes:jpeg,jpg,png|max:10000',     //5MB
                    'cover_image_url' => 'sometimes|required',
                    'business_category' => 'sometimes|required',
                    'business_name' => 'sometimes|required',
    ...
    public function beforeStoreLoadModel($request)
    {
        ...
        if($request->user_role == 2){
            if (empty($request->keywords)){
                $this->__is_error = true;
                return $this->__sendError(__('app.validation_msg'),['message' => __('app.keyword_field_required') ],400);
            }
        }
    }
    ...
    public function login()
    {
        $request = $this->__request;
        $custom_messages = [
            'email.exists' => __('app.email_not_exists'),
        ];
        $param_rule['email']        = 'required|email|exists:users,email';
...
//            }
        ...
        if( $user->user_group_id == 2 && $user->is_approve != 1){
            return $this->__sendError(__('app.validation_msg'),['message' => __('app.account_not_approved')] ,400);
        }
    ...
    public function userEmailOrPhoneExists(Request $request){

        $param_rule['email']     = 'unique:users,email';
        $param_rule['mobile_no'] = 'unique:users,mobile_no';
        if(isset($request->email)){
           $user =  User::emailExists($request);
        }

        if(isset($request->mobile_no)){
            $user =  User::phoneExists($request);
        }

        $response = [
            'code'       => 200,
            'data'       => $user,
            'message'    => __('app.email_or_mobile_exists'),
        ];
    ...
    public function forgotPassword()
    {
        $request = $this->__request;
        $custom_messages = [
            'email.exists' => __('app.email_invalid'),
        ];
        $param_rule['email'] = 'required|email|exists:users,email';
        ...
        if( env('VERIFICATION_TYPE') != 'none'){
            if( env('VERIFICATION_TYPE') == 'email' && $record->is_email_verify != 1 && $record->user_group_id != 2){
                return $this->__sendError(__('app.validation_msg'),['message' => __('app.email_not_verified')], 400);
            }
        }
